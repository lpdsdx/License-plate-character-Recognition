...
fid=fopen('feature33.txt','w+');%打开feature3.txt，用以储存特征3的数据
for k=1001:2000
    ...
    for i=1:a
        for j=1:b
            if(B(i,j)==0)
                C(1,i)=j-1;%由左边起，遇到黑点，-1则计算出字符与左边距离
                break;
            end
            if(j==b && B(i,j)==1)
...
%         C(1,i) = C(1,i) / b;
    ...
    for i=1:a
        for j=b:-1:1 %从右向左
            if(B(i,j)==0)
                C(1,a+i)=b-j;%由右边起，遇到黑点，计算出字符与右边距离
                break;
            end
            if(j==1 && B(i,j)==1)
...
%         C(1,a+i) = C(1,a+i) / a;
    ...
    for j=1:b
        for i=1:a
            if(B(i,j)==0)
                C(1,a*2+j)=i-1;%由上边起，遇到黑点，计算出字符与右边距离
                break;
            end
            if(i==a && B(i,j)==1)
...
%         C(1,a*2+j) = C(1,a*2+j) / a;
    ...
    for j=1:b
        for i=a:-1:1
            if(B(i,j)==0)
                C(1,a*2+b+j)=a-i;%由下边起，遇到黑点，计算出字符与右边距离
                break;
            end
            if(i==1 && B(i,j)==1)
