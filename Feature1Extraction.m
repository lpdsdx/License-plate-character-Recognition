...
[input1, input2, input3] = textread('Char_Index_kuochong.txt','%d %d %s',1000, 'headerlines',1);%读取图片的编号，类别信息和文件名
indexFileName = input3;%获得文件名
fid=fopen('feature1.txt','w+');%打开feature1.txt，用以储存特征1的数据，fid用于存储文件句柄值，如果返回的句柄值大于0，则说明文件打开成功
for k=1:1000 %共1000个图片
    A=imread(strcat('Char_Image_Binary/',char(indexFileName(k,1))));%读入图片
    t=graythresh(A); %取阈值
    B=im2bw(A,t);%二值化，B为二值化后的图像矩阵，每个元素的值为0或1
    [a,b]=size(B);
    C=zeros(1,a+b);%定义特征向量，有a+b维
...
%         C(1,a+j)=C(1,a+j)/a;%归一化，因为行跟列数量差别大
    end
